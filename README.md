...
- 생애주기, 투자 성향, 연금 계좌 종류, 인컴 선호도, 운용 방식을 입력받습니다.
- FastAPI가 입력값을 미래에셋 연금투자 가이드북의 연금 포메이션 표에 매핑합니다.
...
| 인컴 선호도 | 성장 중심, 배당·분배금 선호 |
| 운용 방식 | 직접 조합형, 자동 관리형 |

## 추천 로직 요약

1. 생애주기와 투자 성향을 가이드북의 포메이션 표에 매핑합니다.
2. 20~30대는 연금 성장기 성장형 또는 밸런스형을 적용합니다.
3. 40~50대는 연금 성숙기 밸런스형 또는 안정형을 적용합니다.
4. 60대 이상은 연금 인출기 밸런스형 또는 안정형을 적용합니다.
5. 인컴 선호도와 운용 방식은 비중을 임의로 바꾸지 않고 상품군 안내에 반영합니다.
6. 계좌가 퇴직연금 DC 또는 IRP이면 공격수+미드필더 합산 비중이 70% 이내인지 확인합니다.
...
```json
{
  ...
  "risk_tolerance": "균형형",
  "account_type": "IRP",
  "income_preference": "배당·분배금 선호",
  "management_style": "직접 조합형"
}
```
...
```json
{
  "profile_title": "연금 성숙기 밸런스형 포트폴리오 (균형형, 인컴 선호, 직접조합형)",
  ...
  "allocation": [
    {
      "asset_class": "공격수",
      "ratio": 30,
      "role": "수익 성장을 담당하는 주식형·테마형 자산"
    },
    {
      "asset_class": "미드필더",
      "ratio": 30,
      "role": "성장과 방어의 균형을 잡는 대표지수·배당·분산형 자산"
    },
    {
      "asset_class": "수비수+골키퍼",
      "ratio": 40,
      "role": "변동성 완충과 계좌 안정성을 담당하는 채권·금현물·TDF/TIF형 자산"
    }
  ]
}
```
